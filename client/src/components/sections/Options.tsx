  ...
  return (
    <Section className="border-t border-border">
      <div className="max-w-4xl mx-auto">
        ...
        <StaggerChildren className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-4">
          {options.map((opt, index) => (
            <FadeIn key={index} className="flex justify-between items-center py-3 border-b border-border/50">
              <span className="text-foreground">{opt.label}</span>
              <span className="font-bold text-muted-foreground text-sm">à partir de {opt.price}</span>
            </FadeIn>
          ))}
        </StaggerChildren>
        <FadeIn delay={0.3} className="text-center text-muted-foreground mt-8 text-sm italic">
